...
export default function PurchaseReturnBillList(props) {
    ...
    const callBackEditAction = useCallback((event)=>{
        props.actions.editPurchaseReturnBill({
            'id': event.currentTarget.getAttribute('value1'),
            'purchasebill_id': event.currentTarget.getAttribute('value2'),
        });
    }, []);

    const callBackDeleteAction = useCallback((event)=>{
        props.actions.deletePurchaseReturnBill(event.currentTarget.value);
    });

    const callBackViewAction = useCallback((event)=>{
        props.actions.viewPurchaseReturnBill({
            'id': event.currentTarget.getAttribute('value1'),
            'purchasebill_id': event.currentTarget.getAttribute('value2'),
        });
    });
                                        ...
                                        <IconButton 
                                            ...
                                            :
                                                <>
                                                    ...
                                                    size="small" 
                                                    disabled
                                                    value={{'id':bill.id,'purchasebill_id':bill.purchasebill_id}} 
                                                    children={<EditIcon sx={{cursor:'pointer', fontSize:'16px'}} />}
                                                    ></IconButton>
